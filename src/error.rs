...
pub enum Error {
    ...
    #[error("io: {0}")]
    Io(#[doc = "The I/O error that occurred."] #[from] #[source] std::io::Error),
    ...
    TokioJoin(#[doc = "The join error that occurred."] #[from] tokio::task::JoinError),
    /// Indicates that something is closed.
    #[error("{}", if *.0 { "was closed by operation" } else { "already closed" })]
    Closed(#[doc = "Whether it was closed by the operation (`true`) or was already closed (`false`)"] bool)
}
